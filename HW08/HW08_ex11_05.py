...
# a sorted version of the dict (print key/value pairs from 1 through the
...
#   - Speed comparisons with %timeit are available in the associated .ipynb.
#   - One version demonstrates iteritems() instead of items()
#     For an introduction see "What is the difference between dict.items() and
#     dict.iteritems()?" on StackOverflow.
...
def invert_dict_new_setdefault_style(d):
    inverse = {}
    # .iteritems is a generator (.items in Python 3) and generates the
    # key/value tuples as iterated. Whereas .items in Python 2 creates the
    # entire list of tuples at once, possibly at a memory cost. (docs 5.8)
